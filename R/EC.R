...
EC <- function(X, Y) {
  df <- data.frame(Y = Y, X = I(as.matrix(X)))
  p <- plsr(Y ~ X, data = df)
  proj <- p$projection[, 1]
  #comp <- sqrt(sum(proj[1:2, 1]^2))/sqrt(sum(proj[, 1]^2)) # how much of PLScomp1 falls in PCO comp1+2
  Yvar <- 1-MSEP(p)$val[1, 1, 2]/max(MSEP(p)$val[1, 1, ])
  return(list(proj = proj, Yvar = Yvar))
